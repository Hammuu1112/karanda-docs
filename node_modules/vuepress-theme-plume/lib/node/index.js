...
import { fs as fs5, hash as hash2, path as path8 } from "vuepress/utils";
...
import fs2, { constants, promises as fsp2 } from "node:fs";
...
function existsSync(fp) {
  try {
    fs2.accessSync(fp, constants.R_OK);
    return true;
  } catch {
    return false;
  }
...
function watchConfigFile(app, watchers, onChange) {
  ...
  const watcher = watch(loader.configFile, {
    ignoreInitial: true,
    cwd: path4.join(path4.dirname(loader.configFile), "../")
  });
...
import { fs as fs3, path as path5 } from "vuepress/utils";
...
async function readMarkdown(sourceDir, relativePath) {
  ...
  return {
    filepath,
    relativePath,
    content: await fs3.promises.readFile(filepath, "utf-8"),
...
function initAutoFrontmatter(localeOptions, autoFrontmatter = {}) {
  ...
  async function updateCache(app) {
    if (!isEmptyObject(cache4)) {
      await fs5.mkdir(path8.dirname(app.dir.cache(CACHE_FILE)), { recursive: true });
      await fs5.writeFile(app.dir.cache(CACHE_FILE), JSON.stringify(cache4), "utf-8");
    }
  }
  generate = {
    globFilter,
...
async function generateAutoFrontmatter(app) {
  ...
  if (fs5.existsSync(cachePath)) {
    try {
      generate.cache = JSON.parse(await fs5.readFile(cachePath, "utf-8"));
    } catch {
      generate.cache = {};
    }
  }
...
import { fs as fs6 } from "vuepress/utils";
...
async function prepareIcons(app, localeOptions) {
  ...
  const collectMap = {};
  uniq3(iconList).filter((icon) => !cache2[icon]).forEach((iconName) => {
...
async function readJSON(filepath) {
  try {
    return await fs6.readJSON(filepath, "utf-8");
  } catch {
    return null;
  }
}
