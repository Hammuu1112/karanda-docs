...
export const sassPalettePlugin = (options) => (app) => {
    ...
    const { id = '', config = `.vuepress/styles/${getIdPrefix(id)}config.scss`, defaultConfig = path.resolve(__dirname, '../../styles/default/config.scss'), palette = `.vuepress/styles/${getIdPrefix(id)}palette.scss`, defaultPalette, generator = EMPTY_FILE, style = '', } = options;
    ...
    return {
        ...
        onWatched: (_, watchers) => {
            ...
            const updateConfig = () => prepareConfigSass(app, id, defaultConfig, userConfig).then(() => {
                if (app.env.isDebug)
